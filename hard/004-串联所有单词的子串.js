...
var findSubstring = function (s, words) {
  ...
  const oneLength = words[0].length
  const map = {}
  ...
  for (let i = 0; i < oneLength; i++) {
    ...
    let tempMap = {}
    let count = 0
    while(R <= s.length - oneLength){
      const word = s.slice(R, R + oneLength)
      R += oneLength
      if(!map[word]){
        L = R
        tempMap = {}
      ...
      tempMap[word] ? tempMap[word]++ : tempMap[word] = 1
      count++
      while(tempMap[word] > map[word]){
        const str = s.slice(L, L + oneLength)
        L += oneLength
        tempMap[str]--
        count--
      }
